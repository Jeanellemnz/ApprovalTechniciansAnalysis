...
# Added .copy() here to prevent Pandas warnings
...
with tab2:
    st.subheader(f"Approval Duration Distribution (Gaps <= {int(MaxDuration/60)} minutes)")
...
with tab4:
    ...
    # Added Mutually Exclusive Bins Analysis
    st.markdown("---")
    st.subheader("Mutually Exclusive Approval Time Brackets (%)")
    Bins = [-1, 2, 5, 10, 30, 60, float('inf')]
    BinsLabels = ["0-2 sec", "2-5 sec", "5-10 sec", "10-30 sec", "30-60 sec", "60+ sec"]
    
    WorkingData = WorkingBlocksData.copy()
    WorkingData["DurationBin"] = pd.cut(WorkingData["ApprovalDuration"], bins=Bins, labels=BinsLabels)
    
    PercentME = (WorkingData.groupby("PROVIDER_APPROVING_NAME")["DurationBin"].value_counts(normalize=True).unstack() * 100).round(2)
    st.dataframe(PercentME)
...
with tab5:
    ...
    if not DailyEarnings.empty:
        ...
        # Added Gary Arnold Shift Over Time Scatterplot
        st.markdown("---")
        st.subheader("Gary Arnold Approval Duration Shift Over Time")
        fig_shift, ax_shift = plt.subplots(figsize=(12, 6))
        
        sns.scatterplot(data=DailyEarnings, x="ShiftStart", y="AverageDuration", hue="PayRate", palette="Set1", alpha=0.7, ax=ax_shift)
        ax_shift.axvline(pd.to_datetime("2020-06-01"), color="black", linestyle="--", linewidth=2, label="June 2020 Pay Cut")
        
        ax_shift.set_xlabel("Shift Date", weight="bold")
        ax_shift.set_ylabel("Average Approval Duration (Seconds)", weight="bold")
        ax_shift.legend(title="Pay Rate ($)")
        
        st.pyplot(fig_shift, use_container_width=False)
...
with tab6:
    ...
    # Updated Correlation Matrix with ClaimedPrepTime
    st.subheader("Correlation Matrix")
    CaseCorr = BlockAnalysis[["FatigueIndex", "HighRiskRatio", "AverageDuration", "ClaimedPrepTime"]].dropna()
    ...
    st.markdown("---")
    st.subheader("Regression Analysis: Predictors of High-Risk Approvals")
    Features = ["FatigueIndex", "BlockDensityCPM", "ClaimedPrepTime", "LateNightApprovals"]
    ModelData = BlockAnalysis[["HighRiskRatio"] + Features].dropna()
    
    if not ModelData.empty:
        X = sm.add_constant(ModelData[Features].astype(float))
        y = ModelData["HighRiskRatio"].astype(float)
        
        ModelResults = sm.OLS(y, X).fit()
        st.text(ModelResults.summary().as_text())
...
# Added Welch's Two-Sample T-Test
st.markdown("---")
st.subheader("Before vs. After Pay Cut (Welch's Two-Sample T-Test)")

BeforeCut = BlockAnalysis[BlockAnalysis["PayRate"] == custom_pre_rate]["AverageDuration"].dropna()
AfterCut = BlockAnalysis[BlockAnalysis["PayRate"] == custom_post_rate]["AverageDuration"].dropna()

if len(BeforeCut) > 1 and len(AfterCut) > 1:
    TestResults = stats.ttest_ind(BeforeCut, AfterCut, equal_var=False)
    
    t_col1, t_col2, t_col3 = st.columns(3)
    t_col1.metric(f"Avg Speed BEFORE (${custom_pre_rate})", f"{BeforeCut.mean():.2f} sec")
    t_col2.metric(f"Avg Speed AFTER (${custom_post_rate})", f"{AfterCut.mean():.2f} sec")
    t_col3.metric("P-Value", f"{TestResults.pvalue:.2e}")
    
    if TestResults.pvalue < 0.05 and AfterCut.mean() < BeforeCut.mean():
        st.error("🚨 Statistically significant speed INCREASE detected after the pay cut.")
    else:
        st.success("No statistically significant speed change detected.")

# Added Suspicious Batch Reviews
st.markdown("---")
st.subheader("Top Suspicious Batch Reviews (Disputed Prep Times)")
st.caption("Blocks with massive gaps between cases resulting in impossibly fast 'Average Claimed Prep' per case.")

SortedSummary = BlockAnalysis[['BlockID', 'PROVIDER_APPROVING_NAME', 'ClaimedPrepTime', 'TotalCases', 'ClaimedApprovalAverage']].dropna(subset=['ClaimedApprovalAverage']).sort_values(by='ClaimedApprovalAverage', ascending=True)
st.dataframe(SortedSummary.head(10), hide_index=True)
